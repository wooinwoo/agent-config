...
function emptyBoard() {
  return { note: { content: '', updatedAt: 0 }, tasks: [], nextTaskNumber: 1, revision: 0, updatedAt: 0 };
}
...
function normalizeBoard(input) {
  ...
  const noteContent = typeof source.note?.content === 'string' ? source.note.content.slice(0, 12_000) : '';
  const seen = new Set();
  let highest = 0;
  const tasks = [];
  ...
  return {
    note: { content: noteContent, updatedAt: Number(source.note?.updatedAt) || 0 },
    tasks,
...
export function createBoardService(filePath = BOARD_FILE) {
  ...
  return {
    ...
    updateBoardNote(content) {
      ...
      return mutate(next => {
        next.note = { content: content.slice(0, 12_000), updatedAt: Date.now() };
        return { board: next, result: saved => snapshot(saved) };
      });
    },
    appendBoardNote(content) {
      ...
      return mutate(next => {
        const separator = next.note.content ? '\n' : '';
        if (next.note.content.length + separator.length + extra.length > 12_000) {
          throw new BoardError('content exceeds the 12000 character note limit', 'BOARD_VALIDATION');
        }
        next.note = {
          content: `${next.note.content}${separator}${extra}`,
          updatedAt: Date.now(),
        };
        return { board: next, result: saved => snapshot(saved) };
      });
